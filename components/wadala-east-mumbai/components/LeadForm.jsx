...
import { PROJECT_ID, PROJECT_NAME, API_ENDPOINT, SHEET_NAME, SECRET_KEY, CITY_DISPLAY } from '../../../lib/wadala-east-mumbai/config'
import { buildTrackingFields } from '../../../lib/wadala-east-mumbai/formMeta'
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-1 w-full">
      ...
      {/* <div className="flex items-start gap-2 mt-3">
        ...
        <label htmlFor="privacy-lead" className={`text-xs leading-relaxed cursor-pointer ${isTransparent ? 'text-gray-400' : 'text-gray-500'}`} style={{ fontFamily: F_SANS }}>
          I agree to receive updates as per the <Link href="/the-address-by-gs-wadala/privacy-policy" className={`underline ${isTransparent ? 'hover:text-white' : 'hover:text-[var(--color-gold)]'}`} title="Read our Privacy Policy">Privacy Policy</Link>
        </label>
      </div> */}
